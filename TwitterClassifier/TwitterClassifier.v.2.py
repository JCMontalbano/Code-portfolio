...
#note: you can distinguish iPhone from Android tweet sources by the presence of
#fancy “quotes” which point inward, rather that "quotes" which are the same character.
#change to the following filter string to eliminate them too: 
#bigString1 = bigString1.translate(str.maketrans('','','1234567890“”’'))
...
#problem here: many most-common words are turning up in both. Counter-filter down the lists to eliminate this problem.
...
#save vectorized DF
#generate a dynamic DF name:
now=datetime.datetime.now().strftime('%m%d%Y')
DF_name=target1+'.'+target2+'.'+str(now)+'.vectorDF.csv'
VectorDF.to_csv(DF_name) #save VectorDF
DF=VectorDF
...
#Presently I have this code duplicated above, because it was easy to cross-stitch the data 
#objects as they were produced.
